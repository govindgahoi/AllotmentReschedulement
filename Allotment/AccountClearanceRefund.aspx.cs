...
namespace Allotment
{
    public partial class AccountClearanceRefund : System.Web.UI.Page
    {
        ...
        private void BindServiceRequestGrid()
        {
            ...
            try
            {
                ds = objServiceTimelinesBLL.GetListOfApplicationForRefundClearance(objServiceTimelinesBEL);
        ...
        private void BindServiceRequestClearedGrid()
        {
            ...
            try
            {
                ds = objServiceTimelinesBLL.GetListOfApplicationRefundCleared(objServiceTimelinesBEL);
        ...
        protected void ApplicationGrid_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                GridView gvOrders = e.Row.FindControl("GridViewAccountDetails") as GridView;

                objServiceTimelinesBEL.ServiceRequestNO = reqid;
                DataSet ds = new DataSet();
                try
                {
                    ds = objServiceTimelinesBLL.GetRefundAccountDetails(objServiceTimelinesBEL);
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                e.Row.Cells[2].Attributes.Add("onclick", "ShowPopupChange('" + HttpContext.Current.Server.HtmlDecode(e.Row.Cells[11].Text) + "','Refund Clearence','" + HttpContext.Current.Server.HtmlDecode(e.Row.Cells[2].Text) + "','" + HttpContext.Current.Server.HtmlDecode(e.Row.Cells[5].Text) + "'); return false;");
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txtRefundMode.Text == "")
                {
                    System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "call_show", "ShowPopup('Please Provide Refund Mode');", true);
                    return;
                }
                if (txtRefNo.Text == "")
                {
                    System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "call_show", "ShowPopup('Please Provide Transaction Ref No');", true);
                    return;
                }
                ...
                if (gm.ValidateDate(txtReceivedDate.Text))
                {
                    ...
                    objServiceTimelinesBEL.PayMode = txtRefundMode.Text;
                    objServiceTimelinesBEL.ReferenceNumber = txtRefNo.Text;
                    ...
                    int result = objServiceTimelinesBLL.UpdateApplicationAfterRefundClearence(objServiceTimelinesBEL);
                    if (result > 0)
                    {
                        System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "call_show", "alert('Refund Updated Successfully');", true);
                        BindServiceRequestGrid();
                        BindServiceRequestClearedGrid();
                    }
                    else
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("sp_GetApplicationsForRefundPrint"))
                    {
                        using (SqlDataAdapter sda = new SqlDataAdapter())
                        {
                            ...
                            using (DataTable dt = new DataTable())
                            {
                                ...
                                using (XLWorkbook wb = new XLWorkbook())
                                {
                                    ...
                                    Response.AddHeader("content-disposition", "attachment;filename=PendingRefund" + DateTime.Now.ToString("dd-MMM-yyyy") + ".xlsx");
