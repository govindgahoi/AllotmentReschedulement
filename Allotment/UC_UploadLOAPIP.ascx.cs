...
namespace Allotment
{
    public partial class UC_UploadLOAPIP : System.Web.UI.UserControl
    {
        SqlConnection con;
            ...
            try
            {
                ...
                if (ddlLetterType.SelectedValue.Trim() == "3")
                {
                    using (con)
                    {
                        ...
                        SqlCommand cmd = new SqlCommand("select ServiceRequestID, UID from ServiceRequests where NMServiceID = 'SC21044' and UID !='' and ServiceRequestNO ='" + ServiceReqNo + "'", con);
                        SqlDataAdapter adp = new SqlDataAdapter(cmd);
                        DataTable dt = new DataTable();
                        adp.Fill(dt);
                        if (dt.Rows.Count > 0)
                        ...
                        {
                            SqlCommand cmdAll = new SqlCommand("select ServiceRequestID, UID from ServiceRequests where NMServiceID = 'SC21044' and UID !='' ", con);
                            SqlDataAdapter adpAll = new SqlDataAdapter(cmdAll);
                            ...
                            if (dtAll.Rows.Count > 0)
                            {
                                ...
                                command.CommandText = ("Update ServiceRequests set UID = CONCAT('UID/PIP/', (SELECT REPLACE(REPLACE(REPLACE(REPLACE(CompanyName, ' ', ''), '.', ''), '\', ''), '/', '')  FROM AllotteeMaster WHERE serviceReqNo ='" + ServiceReqNo + "'), '/', (select max(StatuID)+1 from ServiceRequests)), StatuID = (select max(StatuID)+1 from ServiceRequests) where ServiceRequestNO ='" + ServiceReqNo + "'");
                                if (command.ExecuteNonQuery() > 0)
                                {

                                    SqlCommand cmd1 = new SqlCommand("SELECT UID FROM ServiceRequests WHERE ServiceRequestNO ='" + ServiceReqNo + "'", con);
                                    SqlDataAdapter adp1 = new SqlDataAdapter(cmd1);
                                    DataTable dt1 = new DataTable();
                                    adp1.Fill(dt1);
                                    if (dt1.Rows.Count > 0)
                                    {
                                        lblUid.Text = string.Concat("<strong>UNIQUE ID:</strong> ", dt1.Rows[0][0].ToString());
                                    }
                                }
                            }
                            else
                            {                                
                                ...
                                SqlCommand command = con.CreateCommand();
                                command.Connection = con;
                                command.CommandText = ("Update ServiceRequests set UID = CONCAT('UID/PIP/', (SELECT REPLACE(REPLACE(REPLACE(REPLACE(CompanyName, ' ', ''), '.', ''), '\', ''), '/', '')  FROM AllotteeMaster WHERE serviceReqNo ='" + ServiceReqNo + "'), '/1'), StatuID = '1' where ServiceRequestNO ='" + ServiceReqNo + "'");
                                if (command.ExecuteNonQuery() > 0)
                                {
        ...
        protected void btnupload_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (fileupload1.HasFile)
                {
                    ...
                    bool exist = Directory.EnumerateFiles(Server.MapPath("PIPCorrespondenceLetters/MeetingMinutes"), filename).Any();
                    ...
                    if (contenttype != String.Empty && ext == ".pdf")
                    {
                        ...
                        Session["Doc"] = bytes;
                        //fileupload1.SaveAs(Server.MapPath("~/PIPCorrespondenceLetters/LOA/" + Convert.ToString(Session["Name"])));
                        string filename1 = Path.GetFileName(fileupload1.PostedFile.FileName);
                        fileupload1.PostedFile.SaveAs(Server.MapPath("~/PIPCorrespondenceLetters/LOA/" + filename1));
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string filePath = "~/PIPCorrespondenceLetters/LOA/" + Convert.ToString(Session["Name"]);
                ...
                {
                    int retVal = objServiceTimelinesBLL.IssueLetterPIP(objServiceTimelinesBEL);
                    if (retVal > 0)
                    {
