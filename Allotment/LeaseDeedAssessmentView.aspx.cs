...
namespace Allotment
{
    public partial class LeaseDeedAssessmentView : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    cs.str = "SELECT AllotteeID FROM ServiceRequests WHERE ServiceRequestNO='"+ ServiceReqNo + "'";
                    int allottes = Convert.ToInt32(cs.ExecuteScaler(cs.str));
                    cs.str = " SELECT  count(*) FROM  tbl_AllotteePaymentJournal t INNER join AllotteeMaster A  ON t.AllotteeID = a.AllotteeID   inner join IndustrialArea B on A.IndustrialArea = B.IAName where A.AllotteeID = '"+ allottes + "' and ModifiedOn   BETWEEN '1/1/2022' and '12/31/2022' AND isnull(t.Debit, 0)> 0";
                    int DRCount = Convert.ToInt32(cs.ExecuteScaler(cs.str));                   
                        if (DRCount > 0)
                        {
                        ...
                        {
                            System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "call_show", "alert('First update the Allottee Ledger for year 2022');", true);
                            Response.Redirect("ServiceRequestUnderProcessLeaseDeed.aspx");
                        }
