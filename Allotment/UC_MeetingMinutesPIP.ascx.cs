...
namespace Allotment
{
    public partial class UC_MeetingMinutesPIP : System.Web.UI.UserControl
    {
        SqlConnection con;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (btnSubmit.Text == "Save")
                {
                    ...
                    if (txtMeetingDate.Text.Length <= 0)
                    {
                        MessageBox1.ShowWarning("Enter Meeting Date");
                        return;
                    }
                    if (txtDescription.Text.Length <= 0)
                    {
                        MessageBox1.ShowWarning("Enter Meeting Description");
                        return;
                    }
                    ...
                    string filePath = "~/PIPCorrespondenceLetters/MeetingMinutes/" + Convert.ToString(Session["Name"]);
                    string date_to_be = DateTime.ParseExact(txtMeetingDate.Text.Trim(), "dd/MM/yyyy", CultureInfo.InvariantCulture).ToString("dd MMM yyyy", CultureInfo.InvariantCulture);

                    objServiceTimelinesBEL.LetterFrom = (ddlMeetingType.SelectedItem.Text.Trim());
                    objServiceTimelinesBEL.LetterDate = Convert.ToDateTime((date_to_be));
                    objServiceTimelinesBEL.Remarks = txtDescription.Text;
                    ...
                    int retVal = objServiceTimelinesBLL.InsertMeetingMinutesPIP(objServiceTimelinesBEL);
                    if (retVal > 0)
                    {
                ...
                if (btnSubmit.Text == "Update")
                {
                    ...
                    if (Convert.ToString(Session["Name"]).Length > 0)
                    {
                        filePath = "~/PIPCorrespondenceLetters/MeetingMinutes/" + Convert.ToString(Session["Name"]);
                    }
                    else
                    {
                    ...
                    objServiceTimelinesBEL.CreatedBy = UserName.Trim();
                    objServiceTimelinesBEL.ServiceRequestNO = ServiceReqNo.Trim();
                    int retVal = objServiceTimelinesBLL.UpdateMeetingMinutesPIP(objServiceTimelinesBEL);
                    if (retVal > 0)
                    {
                        if (Convert.ToString(Session["Name"]).Length > 0)
                        {
        ...
        private void BindOfficeOrders()
        {
            ...
            try
            {
                ...
                ds = objServiceTimelinesBLL.GetAllMeetingMinutesPIP(objServiceTimelinesBEL);
        ...
        protected void GridView2_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "EditRow")
            {
                ...
                ddlMeetingType.ClearSelection();
                lblServiceID.Text = GridView2.DataKeys[index].Values[0].ToString();
                lblFilePath.Text = GridView2.DataKeys[index].Values[1].ToString();
                txtMeetingDate.Text = GridView2.Rows[index].Cells[2].Text;
                txtDescription.Text = GridView2.Rows[index].Cells[3].Text;

                var b1 = GridView2.Rows[index].Cells[1].Text;
                ddlMeetingType.SelectedIndex = ddlMeetingType.Items.IndexOf(ddlMeetingType.Items.FindByText(b1));

                //ddlMeetingType.Items.FindByText(GridView2.Rows[index].Cells[1].Text).Selected = true;
            ...
            if (e.CommandName == "DeleteRow")
            {
                ...
                try
                {
                    int retVal = objServiceTimelinesBLL.DeleteMeetingMinutesPIP(objServiceTimelinesBEL);

                    if (retVal > 0)
                    {
                        File.Delete(Server.MapPath(filePath));
                        string msg = "Minutes deleted successfully";
                        System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "call_show", "alert('" + msg + "');", true);
