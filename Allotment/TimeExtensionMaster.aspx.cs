...
namespace Allotment
{
    public partial class TimeExtensionMaster : System.Web.UI.Page
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    MessageBox1.ShowWarning("Please Provide Time Extension Rate");
                    return;
                }
                ...
                command = new SqlCommand(@"[InsertTimeExtensionMaster_sp]", con, transaction);
                ...
                {
                    transaction.Commit();
                    string message = "Time Extension  detail Updated/Inserted Successfully.";
        ...
        private void bindRateGrid(string IAId, string p)
        {
            ...
            try
            {
                ds = objServiceTimelinesBLL.GetTimeExtensionPerDetails(objServiceTimelinesBEL);
        ...
        {
            SqlCommand cmd = new SqlCommand("[GetIATimeExtensionDetail_sp2] '" + reg + "','" + ia + "','" + txtSearch.Text.Trim() + "'", con);
            SqlDataAdapter adp = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            adp.Fill(dt);
            IARateGrid.DataSource = dt;
        ...
        protected void IARateGrid_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "DeleteRate")
            {
                ...
                int retVal = objServiceTimelinesBLL.DeleteIATimeExtension(objServiceTimelinesBEL);
                if (retVal > 0)
                {
                    MessageBox1.ShowSuccess("Time Extension Deleted Successfully");
                    gridFunc();
                }
                else
                {
        ...
        private void get_prev_rate(int rowid)
        {
            ...
            try
            {
                ds = objServiceTimelinesBLL.GetIATimeExtensionPrev(objServiceTimelinesBEL);
                DataTable dt = ds.Tables[0];
                if (dt != null)
                {
                    ...
                    {
                        txtEffectiveFromDate.Text = dt.Rows[0]["effective_from1"].ToString().Trim();
                    ...
                    {
                        txtEffectiveToDate.Text = dt.Rows[0]["effective_to1"].ToString().Trim();
