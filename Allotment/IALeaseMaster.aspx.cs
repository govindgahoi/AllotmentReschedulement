...
namespace Allotment
{
    public partial class IALeaseMaster : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                //txtEffectiveFromDate.Attributes.Add("ReadOnly", "ReadOnly");
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                command = new SqlCommand(@"[InsertIALeaseRateMaster_sp]", con, transaction);
                ...
                {
                    transaction.Commit();
                    string message = "Lease Rate detail Updated/Inserted Successfully.";
        ...
        private void bindRateGrid(string IAId, string p)
        {
            ...
            try
            {
                ds = objServiceTimelinesBLL.GetIALeaseRateDetails(objServiceTimelinesBEL);
        ...
        {
            SqlCommand cmd = new SqlCommand("[GetIALeaseRatesDetail_sp2] '" + reg + "','" + ia + "','" + txtSearch.Text.Trim() + "'", con);
            SqlDataAdapter adp = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            adp.Fill(dt);
            IARateGrid.DataSource = dt;
        ...
        protected void IARateGrid_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "DeleteRate")
            {
                ...
                int retVal = objServiceTimelinesBLL.DeleteIALeaseRate(objServiceTimelinesBEL);
                if (retVal > 0)
                {
        ...
        private void get_prev_rate(int rowid)
        {
            ...
            try
            {
                ds = objServiceTimelinesBLL.GetIALeaseRatePrev(objServiceTimelinesBEL);
                DataTable dt = ds.Tables[0];
                if (dt != null)
                {
        ...
        private void get_min_period(string ia)
        {
            RateIdlbl.Text = "0";
            objServiceTimelinesBEL.IAName = ia;
            ...
            try
            {
                ds = objServiceTimelinesBLL.GetIAMinPeriodLease(objServiceTimelinesBEL);
                DataTable dt = ds.Tables[0];
                if (dt != null)
                {
                    txtMinPeriod.Text = dt.Rows[0][0].ToString();
