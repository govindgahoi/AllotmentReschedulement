...
namespace Allotment
{
    public partial class UC_PIPEvaluation : System.Web.UI.UserControl
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("sp_getPIPEvaluationDetails '" + ServiceReqNo + "'", con);
        ...
        protected void BtnProjectInsert_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                retval = objServiceTimelinesBLL.UpdateEvaluationDetailsPIP(objServiceTimelinesBEL);
                if (retval >= 0)
                {
                    objServiceTimelinesBEL.LAWID = ServiceReqNo.Trim();
                    int retVal2 = objServiceTimelinesBLL.ClearCommentsDescriptionPIP(objServiceTimelinesBEL);
                    if (retVal2 >= 0)
                    {
                        ...
                        if (temp.Rows.Count > 0)
                        {
                            foreach (DataRow dr1 in temp.Rows)
                            {
                                ...
                                retval = objServiceTimelinesBLL.SaveCommentsDescriptionPIP(objServiceTimelinesBEL);
        ...
        private void BindEvaluationData()
        {
            try
            {
                SqlCommand cmd = new SqlCommand("sp_PIPApplicantDataForEvaluation '" + ServiceReqNo + "'", con);
                SqlDataAdapter adp = new SqlDataAdapter(cmd);
                DataSet ds = new DataSet();
                adp.Fill(ds);
        ...
        protected void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            Response.Write("<script>window.open ('PIPEvaluationSheetPrint.aspx?ID=" + ID + "','_blank');</script>");
