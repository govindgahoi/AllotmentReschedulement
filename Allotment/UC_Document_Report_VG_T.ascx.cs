...
namespace Allotment
{
    public partial class UC_Document_Report_VG_T : System.Web.UI.UserControl
    {
        ...
        protected void btnSave_Genrate_Click(object sender, EventArgs e)
        {
            ...
            if (ServiceID == 4)
            {
                ...
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("create_plot_Allotment_Data_Move", con, transaction);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@Service_Request_No", SerRequestNo);
                    cmd.Parameters.AddWithValue("@ContentType", "application/pdf");
                    cmd.Parameters.AddWithValue("@DocumentInByte", PdfInBytes);
...
<table Class='table table-hover table-bordered request-table' style='width:100%;'>
            ...
            if (ServiceID == 4)
            {
                ...
                try
                {
                    SqlCommand cmd = new SqlCommand("GetTransferLetter '" + SerRequestNo + "'", con);
                    SqlDataAdapter adp = new SqlDataAdapter(cmd);
                    DataSet ds = new DataSet();
                    ...
                    if (dt5.Rows.Count > 0)
                    {
                        ...
                        if (dt4.Rows.Count > 0)
                        {
                            string lblPlotNo = dt3.Rows[0]["PlotNo"].ToString();
                            string lblPlotSize = dt3.Rows[0]["PlotSize"].ToString();
                            string lblPlotRate = dt4.Rows[0]["RateOfPlot"].ToString();
                            string lblTotalLevyamount = dt4.Rows[0]["LevyAmount"].ToString();
                            string lblPrePercent = dt4.Rows[0]["PrePercent"].ToString();
                            string lblPreLevyAmt = dt4.Rows[0]["PreAmount"].ToString();
                            string lblbalPercent = dt4.Rows[0]["RemPercent"].ToString();
                            string BalLevyAmt = dt4.Rows[0]["BalLevyAmount"].ToString();
                            string lblInterestPerAnnum = dt4.Rows[0]["InterestRate"].ToString();
                            string lblRebatePer = dt4.Rows[0]["Rebate"].ToString();


                            string lblNoInstallment = dt3.Rows[0]["NoofInstallments"].ToString();
                            string LevyPer = dt3.Rows[0]["Transfer_Levy_Percentage"].ToString();

                            string Premium = dt3.Rows[0]["Premium"].ToString();
                            string IntOnPremium = dt3.Rows[0]["IntOnPremium"].ToString();
                            string MaintenanceCharge = dt3.Rows[0]["MaintenanceCharge"].ToString();
                            string IntOnMaintenanceCharge = dt3.Rows[0]["IntOnMaintenanceCharge"].ToString();
                            string LeaseRent = dt3.Rows[0]["LeaseRent"].ToString();
                            string GSTOnLeaseRent = dt3.Rows[0]["GSTOnLeaseRent"].ToString();
                            string TEFVerified = dt3.Rows[0]["TEF"].ToString();
                            string IntOnTEF = dt3.Rows[0]["IntOnTEF"].ToString();
                            string LeasePeriod = dt3.Rows[0]["LeasePeriod"].ToString();
                            string TotalBalanceDues = dt3.Rows[0]["TotalBalanceDues"].ToString();

                            string LevyType = dt3.Rows[0]["LeavyType"].ToString();


                            if (LevyType == "Installment")
                            {
                                htmlContent = htmlContent.Replace("<span class='levy_lumpsum'>will be paid by you in lumpsum within 30 days.</span>", "");
                            }
                            if (LevyType == "LumpSum")
                            {
                                htmlContent = htmlContent.Replace("<span class='levy_installment'>out of above levy <b><u class='myultext'><span class='PreviewClass lblPrePercent'>{{lblPrePercent}}</span></u>", "");
                                htmlContent = htmlContent.Replace("</b>% amount of Rs.<b><u class='myultext'><span class='PreviewClass lblPreLevyAmt'>{{lblPreLevyAmt}}</span></u></b>", "");

                                htmlContent = htmlContent.Replace("shall to be paid within 30 days from the date of this letter & balance <b><u class='myultext'><span class='PreviewClass lblbalPercent'>{{lblbalPercent}}</span></u></b>", "");

                                htmlContent = htmlContent.Replace("% of remaining balance is Rs.<b><u class='myultext'><span class='PreviewClass lblbalAmount'>{{lblbalAmount}}</span></u></b> and is to be paid in <b><u class='myultext'>", "");
                                htmlContent = htmlContent.Replace("<span class='PreviewClass lblNoInstallment'>{{lblNoInstallment}}</span></u></b> equal half yearly installments the first of which falls due on <b><u class='myultext'>", "");
                                htmlContent = htmlContent.Replace("<span class='PreviewClass FirstInstallmentDate'>{{FirstInstallmentDate}}</span></u></b> and subsequent installment there on alongwith interest @", "");
                                htmlContent = htmlContent.Replace("<span class='PreviewClass lblInterestPerAnnum'>{{lblInterestPerAnnum}}</span>% of on the balance amount, for timely payment rebate @<b><u class='myultext'>", "");
                                htmlContent = htmlContent.Replace("<span class='PreviewClass lblRebatePer'>{{lblRebatePer}}</span></u></b>% will be admissible.</span>", "");

                            }



                            htmlContent = htmlContent.Replace("{{lblPlotNo}}", lblPlotNo);
                            htmlContent = htmlContent.Replace("{{lblPlotSize}}", lblPlotSize);
                            htmlContent = htmlContent.Replace("{{lblPlotRate}}", lblPlotRate);
                            htmlContent = htmlContent.Replace("{{lblTotalLevyamount}}", lblTotalLevyamount);
                            htmlContent = htmlContent.Replace("{{lblPrePercent}}", lblPrePercent);
                            htmlContent = htmlContent.Replace("{{lblPreLevyAmt}}", lblPreLevyAmt);
                            htmlContent = htmlContent.Replace("{{lblbalPercent}}", lblbalPercent);
                            htmlContent = htmlContent.Replace("{{lblInterestPerAnnum}}", lblInterestPerAnnum);
                            htmlContent = htmlContent.Replace("{{lblRebatePer}}", lblRebatePer);
                            htmlContent = htmlContent.Replace("{{lblNoInstallment}}", lblNoInstallment);
                            htmlContent = htmlContent.Replace("{{lblDue_Premium}}", Premium);
                            htmlContent = htmlContent.Replace("{{lblDue_Intt_Premium}}", IntOnPremium);
                            htmlContent = htmlContent.Replace("{{lblDue_Maint}}", MaintenanceCharge);
                            htmlContent = htmlContent.Replace("{{lblDue_Intt_Maint}}", IntOnMaintenanceCharge);
                            htmlContent = htmlContent.Replace("{{lblDue_Lease}}", LeaseRent);
                            htmlContent = htmlContent.Replace("{{lblDue_GST_Lease}}", GSTOnLeaseRent);
                            htmlContent = htmlContent.Replace("{{lblDue_TEF}}", TEFVerified);
                            htmlContent = htmlContent.Replace("{{lblDue_Intt_TEF}}", IntOnTEF);
                            htmlContent = htmlContent.Replace("{{lblTotalBalanceDues}}", TotalBalanceDues);
                            htmlContent = htmlContent.Replace("{{lbl_LeasePeriod}}", LeasePeriod);
                            htmlContent = htmlContent.Replace("{{lblTransferLevyPer}}", LevyPer);
                            htmlContent = htmlContent.Replace("{{lblbalAmount}}", BalLevyAmt);
                            htmlContent = htmlContent.Replace("{{FirstInstallmentDate}}", FirstInstallmentDate);


                            string code = "ApplicationNo:" + SerRequestNo + "|Transfree:" + transfreeName + "|IAName:" + IANAme + "|PlotNo:" + lblPlotNo + "|DocType:PlotTransfer";
                            QRCodeGenerator qrGenerator = new QRCodeGenerator();
            ...
            if (ServiceID == 1000)
            {
                ...
                try
                {
                    ...
                    if (dt4.Rows.Count > 0)
                    {
                        ...
                        htmlContent = htmlContent.Replace("{{TotalReservationMoneyPer}}", ReservationMoneyVerified);
