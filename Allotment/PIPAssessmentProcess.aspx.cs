...
namespace Allotment
{
    public partial class PIPAssessmentProcess : System.Web.UI.Page
    {
        ...
        protected void MenuMaker()
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    string IsCompleted = dt7.Rows[0]["isCompleted"].ToString();
                    string IsRejected = dt7.Rows[0]["isRejected"].ToString();
                    DocLetterPathLOA = dt7.Rows[0]["DocPath"].ToString();
        ...
        protected void load_UC(string AllOrFirst)
        {
            ...
            try
            {
                ...
                if (Menu_Type == "page")
                {
                    ...
                    {
                        UC_DocumentFinalViewPIP UC_DocumentFinalViewPIP = LoadControl("~/UC_DocumentFinalViewPIP.ascx") as UC_DocumentFinalViewPIP;
                        UC_DocumentFinalViewPIP.ID = "UC_DocumentFinalViewPIP";
                        UC_DocumentFinalViewPIP.ServiceReqNo = ServiceReqNo;
                        Placeholder.Controls.Add(UC_DocumentFinalViewPIP);
                    }
                    ...
                    if (Menu_Value1 == "Rejection" || Menu_Value1 == "LOA")
                    {
                        ...
                        if (dtM.Rows.Count > 0)
                        {
                            DocLetterPathLOA = dtM.Rows[0]["DocPath"].ToString();

                            if (DocLetterPathLOA != "")
                            {
                                string Path = DocLetterPathLOA.Trim();
                                string embed = "<object data=\"{0}\" type=\"application/pdf\" width=\"100%\" height=\"600px\">";
                                embed += "If you are unable to view file, you can download from <a href = \"{0}\">here</a>";
                                embed += " or download <a target = \"_blank\" href = \"http://get.adobe.com/reader/\">Adobe PDF Reader</a> to view the file.";
                                embed += "</object>";
