...
namespace Allotment
{
    public partial class LeaseDeedAssessment : System.Web.UI.Page
    {
        ...
        {
            string RODate = "", SiteInspection = "", Tracing = "", GMIDC = "", LeaseRegistered = "",RMStatus="";
            SqlCommand cmd = new SqlCommand("Select *, (select Top 1 rtrim(ltrim(Status)) from ServiceRequest_Track where Transfer_From='RM' and Transfer_To='DA' and ServiceID='"+ServiceReqNo.Trim()+"' order by Entry_Time desc) 'RMStatus' from tbl_LeaseDeedApplicationStatusMaster where ServiceRequestNo='" + ServiceReqNo + "'", con);
            SqlDataAdapter adp = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            adp.Fill(dt);
            ...
            if (dt.Rows.Count > 0)
            {
                RMStatus = dt.Rows[0]["RMStatus"].ToString();
                RODate = dt.Rows[0]["DateOFAppointmentRO"].ToString();
                SiteInspection = dt.Rows[0]["InspectionReport"].ToString();
                Tracing = dt.Rows[0]["PlotTracing"].ToString();
                ...
                case "1001":
                    {
                        ...
                        if (Level == "DA")
                        {
                            sub_menu.Items.Add(new MenuItem { Value = "page|Select_POA", Text = "Select POA" });
                        ...
                        if (Level == "JE")
                        {
                            sub_menu.Items.Add(new MenuItem { Value = "page|JE_Doc", Text = "Upload Site Inspection Details" });
                        }
                        ...
                        if (Level == "DA")
                        {
                            if (RMStatus=="Recommended")
                            {
                                sub_menu.Items.Add(new MenuItem { Value = "page|UploadLease", Text = "Upload Lease Details" });
                            }
                        }
                        sub_menu.Items.Add(new MenuItem { Value = "page|SendApplication", Text = "Transfer Application" });
            ...
            if (Menu_Type == "page")
            {
                ...
                if (Menu_Value1 == "Payment_Details")
                {
                    ...
                    string TraID = dt.Rows[0]["LATraID"].ToString();
                        ...
                        //UC_LeaseDeedProcessingFee UC_LeaseDeedProcessingFee = LoadControl("~/UC_LeaseDeedProcessingFee.ascx") as UC_LeaseDeedProcessingFee;

                        //UC_LeaseDeedProcessingFee.IndustrialArea = Convert.ToInt32(IAID);
                        //UC_LeaseDeedProcessingFee.choicearea = Convert.ToDouble(PlotArea);
                        //UC_LeaseDeedProcessingFee.CoveredArea = CoveredArea;
                        //UC_LeaseDeedProcessingFee.AllotmentLetterNo = AllottmentNo;
                        //UC_LeaseDeedProcessingFee.SWCControlID = SWCControlID;
                        //UC_LeaseDeedProcessingFee.SWCUnitID = SWCUnitID;
                        //UC_LeaseDeedProcessingFee.SWCServiceID = SWCServiceID;
                        //UC_LeaseDeedProcessingFee.ServiceRequestNo = ServiceReqNo;
                        //UC_LeaseDeedProcessingFee.TranID = TraID;
                        //UC_LeaseDeedProcessingFee.ApplicantName = Applicantname;
                        //UC_LeaseDeedProcessingFee.applicantAddress = Address;
                        //Placeholder.Controls.Add(UC_LeaseDeedProcessingFee);
