...
namespace Allotment
{
    public partial class ProductionStartDetails : System.Web.UI.Page
    {
        ...
        private void bindGridDetail(string IAId, string RegionalOffice, string CompanyId, string Search)
        {
            lblArea.Text = "0";
            lblTotal.Text = "0";
            SqlCommand cmd = new SqlCommand("sp_CM_GetProductionDetailTemp '" + IAId + "','" + RegionalOffice + "','" + CompanyId + "','" + Search + "'", con);
            SqlDataAdapter adp = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            adp.Fill(dt);
        ...
        public void GetProductionDetail()
        {
            try
            {
                ...
                lblArea.Text = "0";
                lblTotal.Text = "0";
                ds = objServiceTimelinesBLL.GetCM_ProductionDetail(objServiceTimelinesBEL);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    //  lblArea.Text = total.ToString();

                    GridView1.FooterRow.Cells[0].Text = "Total";
                    GridView1.FooterRow.Cells[1].HorizontalAlign = HorizontalAlign.Right;
                    GridView1.FooterRow.Cells[6].Text = total.ToString("N2");
                    //GridView1.FooterRow.Cells[4].Text = (GridView1.Rows.Count.ToString());
            ...
            try
            {
                ...
                try
                {
                    if (btnSubmit.Text == "Save")
                    {
                        ...
                        int retVal = objServiceTimelinesBLL.SaveCM_ProductionDetail(objServiceTimelinesBEL);
                        if (retVal > 0)
                        {
                    ...
                    if (btnSubmit.Text == "Update")
                    {
                        ...
                        int retVal = objServiceTimelinesBLL.UpdateCM_ProductionDetail(objServiceTimelinesBEL);
                        if (retVal > 0)
                        {
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                if (e.CommandName == "DeleteVacant")
                {
                    ...
                    try
                    {
                        int retVal = objServiceTimelinesBLL.DeleteCM_ProductionDetail(objServiceTimelinesBEL);
        ...
        private void get_prev_rate(int rowid)
        {
            ...
            try
            {
                ds = objServiceTimelinesBLL.GetCM_PreProductionDetail(objServiceTimelinesBEL);
